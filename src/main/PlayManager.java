...
public class PlayManager {
  ...
  public void update() {
    ...
    if (currentMino.active == false) {
        // if the mino is not active, put it into thew staticBlocks
        ...
        if(currentMino.b[0].x == MINO_START_X && currentMino.b[0].y == MINO_START_Y)  {
          // this means the currentMino immediately collieded a block and couldn't move at all
          // so it's xy are the same with the nextMibo's
          gameOver = true;
        }
      ...
      if(x == right_x)  {
        ...
        if(blockCount == 12){
          ...
          for(int i = staticBlocks.size()-1; i > -1; i --){
            // removbe all the blocks in the current y line
          ...
          // a line has been deleted so need to slid down blocks that are above it
  ...
  public void draw(Graphics2D g2){
    ...
    g2.setFont(new Font("Press Start 2P", Font.PLAIN, 50));
    if(gameOver) {
